...
export default function ChatInterface({ onClose }: ChatInterfaceProps) {
  ...
  return (
    <aside
      key={config.pos}
      class={`fixed ${config.pos === "right" ? "right-margin-desktop" : "left-margin-desktop"} top-margin-desktop bottom-margin-desktop w-panel-width rounded-xl glass-panel shadow-2xl shadow-black/5 flex flex-col overflow-hidden z-50 animate-slide-up`}
    >
      ...
      <div class="flex-1 overflow-y-auto p-6 space-y-6">
        {messages
          .filter(
            (msg) =>
              (msg instanceof AIMessage || msg instanceof HumanMessage) && msg.content !== "",
          )
          .map((msg, i) => (
      ...
      <div class="p-6 pt-0 shrink-0">
        {messages.length === 1 && (
          <div class="flex flex-wrap gap-2 mb-3">
            <button
              onClick={handleTips("click to next section")}
              disabled={!canSend}
              class="px-3 py-1.5 rounded-lg border border-black/5 bg-black/5 hover:bg-black/10 text-on-surface-variant hover:text-on-surface text-[11px] font-label-mono uppercase tracking-wider transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed active:scale-95"
            >
              click to next section
            </button>
            <button
              onClick={handleTips("extract data table")}
              disabled={!canSend}
              class="px-3 py-1.5 rounded-lg border border-black/5 bg-black/5 hover:bg-black/10 text-on-surface-variant hover:text-on-surface text-[11px] font-label-mono uppercase tracking-wider transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed active:scale-95"
            >
              extract data table
            ...
            <button
              onClick={handleTips("find actions")}
              disabled={!canSend}
              class="px-3 py-1.5 rounded-lg border border-black/5 bg-black/5 hover:bg-black/10 text-on-surface-variant hover:text-on-surface text-[11px] font-label-mono uppercase tracking-wider transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed active:scale-95"
            >
              find actions
            ...
            <button
              onClick={handleTips("scrape form fields")}
              disabled={!canSend}
              class="px-3 py-1.5 rounded-lg border border-black/5 bg-black/5 hover:bg-black/10 text-on-surface-variant hover:text-on-surface text-[11px] font-label-mono uppercase tracking-wider transition-all cursor-pointer disabled:opacity-40 disabled:cursor-not-allowed active:scale-95"
            >
              scrape form fields
