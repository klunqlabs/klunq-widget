...
| `npm run build` | `tsc && vite build` — produces `dist/klunq-widget.js` (IIFE) + `dist/klunq-widget.css` |
| `npm run preview` | Preview production build locally |

## Embedding the widget (production)

```html
<link rel="stylesheet" href="klunq-widget.css" />
...
- CSS is imported inline via `?inline` in dev (injected into shadow DOM as `<style>`); for production builds it extracts to `dist/klunq-widget.css` via Vite's `assetFileNames`.
